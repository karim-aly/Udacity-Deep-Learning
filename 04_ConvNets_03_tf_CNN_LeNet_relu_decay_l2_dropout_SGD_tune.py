...
num_steps = 3001
beta_vals = [1e-5, 3e-5, 1e-4, 3e-4, 1e-3, 3e-3, 1e-2, 3e-2]
valid_accuracy_val = []
test_accuracy_val = []

for beta_reg in beta_vals:
  with tf.Session(graph=graph) as session:
    ...
    valid_acc_cur = accuracy(valid_prediction.eval(), valid_labels)
    test_acc_cur = accuracy(test_prediction.eval(), test_labels)
    print('At regularization beta: %f' % valid_acc_cur)
    print('Validation accuracy: %.1f%%' % valid_acc_cur)
    print('Test accuracy: %.1f%%' % test_acc_cur)
    test_accuracy_val.append(test_acc_cur)
    valid_accuracy_val.append(valid_acc_cur)


import matplotlib.pyplot as plt

print(test_accuracy_val)
print(valid_accuracy_val)

plt.semilogx(beta_vals, valid_accuracy_val)
plt.grid(True)
plt.title('Validation accuracy by regularization (logistic)')
plt.show()

plt.figure()
plt.semilogx(beta_vals, test_accuracy_val)
plt.grid(True)
