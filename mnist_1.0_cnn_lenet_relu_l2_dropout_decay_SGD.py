...
with graph.as_default():
    ...
    def model(data, keep_prob):
        data_image = tf.reshape(data, [-1, image_size, image_size, num_channels])

        hidden = tf.nn.relu(conv2d(data_image, layer1_weights) + layer1_biases)
...
with tf.Session(graph=graph) as session:
    ...
    print('Test accuracy: %.1f%%' % accuracy(test_prediction.eval(), test_labels))  # 99.2 %
